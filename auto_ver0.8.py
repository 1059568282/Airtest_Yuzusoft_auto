...
def custom_resize_method(w, h, sch_resolution, src_resolution):
    try:
        ...
        rs_per_x = dev_width / 1280.0
        rs_per_y = dev_height / 720.0
...
def apply_settings():
    global REPEAT_TIMES, MODE, ST, update_settings
    try:
...
def run_automation_logic():
    global REPEAT_TIMES, MODE, ST, MAX_COUNT, script_running, script_paused, update_settings
...
if __name__ == '__main__':
    ...
    pause_button.grid(column=2, row=4, sticky=tk.W)

    stop_button = ttk.Button(mainframe, text="停止", command=lambda: stop_script())
    stop_button.grid(column=3, row=4, sticky=tk.W)
